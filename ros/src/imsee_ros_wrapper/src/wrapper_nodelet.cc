...
class ROSWrapperNodelet : public nodelet::Nodelet
{
  public:
    ...
    {
        //TODO add the publisher for device info
        if (m_device_info_publisher.getNumSubscribers() > 0)
        {
            imsee_ros_wrapper::DeviceInfo msg;
            CameraParameter cam_left = g_params._left_camera[camera_resolution];
            CameraParameter cam_right = g_params._right_camera[camera_resolution];
            // fill the msg
            {
                msg.height = cam_left._height;
                msg.width = cam_left._width;
                msg.camera_fps = img_freq;
                msg.imu_frequency = imu_freq;
                msg.distortion_model = "KannalaBrandt8";
                for (size_t i = 0; i < 16; i++)
                {
                    msg.TSC_c0[i] = cam_left._TSC[i];
                    msg.TSC_c1[i] = cam_right._TSC[i];
                }
                for (size_t i = 0; i < 2; i++)
                {
                    msg.focal_length_c0[i] = cam_left._focal_length[i];
                    msg.principal_point_c0[i] = cam_left._principal_point[i];
                    msg.focal_length_c1[i] = cam_right._focal_length[i];
                    msg.principal_point_c1[i] = cam_right._principal_point[i];
                }

                for (size_t i = 0; i < 4; i++)
                {
                    msg.D_c0[i] = cam_left._D[i];
                    msg.D_c1[i] = cam_right._D[i];
                }
                for (size_t i = 0; i < 9; i++)
                {
                    msg.K_c0[i] = cam_left._K[i];
                    msg.K_c1[i] = cam_right._K[i];
                }
                for (size_t i = 0; i < 9; i++)
                {
                    msg.R_c0[i] = cam_left._R[i];
                    msg.R_c1[i] = cam_right._R[i];
                }
                for (size_t i = 0; i < 12; i++)
                {
                    msg.P_c0[i] = cam_left._P[i];
                    msg.P_c1[i] = cam_right._P[i];
                }
                msg.imu_sigma_g_c = g_params._imu._sigma_g_c;
                msg.imu_sigma_a_c = g_params._imu._sigma_a_c;
                msg.imu_sigma_bg = g_params._imu._sigma_bg;
                msg.imu_sigma_ba = g_params._imu._sigma_ba;
                msg.imu_sigma_gw_c = g_params._imu._sigma_gw_c;
                msg.imu_sigma_aw_c = g_params._imu._sigma_aw_c;
            }
            m_device_info_publisher.publish(msg);
        }
    ...
    void onInit() override
    {
        ...
        m_device_info_publisher = nh_.advertise<imsee_ros_wrapper::DeviceInfo>("/imsee/device_info", 1, true);
        if (m_pSDK)
        {
        ...
        else if (!m_pSDK)
        {
            ...
            imu_freq = config.imuFrequency;
            img_freq = config.imgFrequency;

            switch (index)
            {
                case 1:
                    config.imgResolution = IMG_640;
                    camera_resolution = RESOLUTION::RES_640X400;
                    break;
                case 2:
                    config.imgResolution = IMG_1280;
                    camera_resolution = RESOLUTION::RES_1280X800;
                    break;
                default:
            ...
            LoadCameraInfo(g_params._left_camera[camera_resolution]);
  ...
  private:
    ...
    ros::Publisher m_device_info_publisher;
